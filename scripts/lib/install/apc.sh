...
register_windows_runonce_resume() {
  local resume_cmd
  resume_cmd="wsl.exe -d NixOS --exec env ANIX_APC_RESUME=1 bash -lc \"curl -fsSL ${install_script_url} -o /tmp/install-anix && chmod +x /tmp/install-anix && /tmp/install-anix apc\""
  run_ps "Set-ItemProperty -Path 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\RunOnce' -Name 'anix-apc-resume' -Value '$resume_cmd' -Force"
}

# clear the one-shot windows resume command.
clear_windows_runonce_resume() { run_ps "Remove-ItemProperty -Path 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\RunOnce' -Name 'anix-apc-resume' -ErrorAction SilentlyContinue" || true; }
...
run_apc() {
  if [[ -z "${WSL_DISTRO_NAME:-}" && ! -f /proc/sys/fs/binfmt_misc/WSLInterop ]]; then
    ANIX_KEEP_APC_RESUME_MARKER=0
    trap 'if [[ "${ANIX_KEEP_APC_RESUME_MARKER:-0}" != 1 ]]; then clear_windows_resume_marker || true; clear_windows_runonce_resume || true; fi' EXIT
...
EOF
    ...
    run_ps_admin "wsl --install --no-distribution" || { ANIX_KEEP_APC_RESUME_MARKER=1; echo "note: WSL installation reported a restart is required; Windows should resume this installer after you sign back in." >&2; exit 1; }
    run_ps_admin "wsl --install --from-file \"\$env:TEMP\\nixos.wsl\"" || { ANIX_KEEP_APC_RESUME_MARKER=1; echo "note: NixOS-WSL installation reported a restart or manual completion is required; Windows should try to resume after sign-in." >&2; exit 1; }
    run_ps "wsl -d NixOS --exec /bin/sh -lc 'printf ready'" >/dev/null 2>&1 || { ANIX_KEEP_APC_RESUME_MARKER=1; echo "note: the NixOS-WSL guest is not ready yet; sign back into Windows, start NixOS-WSL, then rerun this script inside the guest if it does not continue automatically." >&2; exit 1; }
    ...
    run_ps "wsl -d NixOS --exec bash -lc 'curl -fsSL ${install_script_url} -o /tmp/install-anix && bash /tmp/install-anix apc'"
    exit $?
  fi
...
EOF
  consent "run nixos-rebuild switch for apc?" || exit 1
