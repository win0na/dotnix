...
EOF
    ...
    run_ps_admin "wsl --install --no-distribution" || { echo "note: WSL installation reported a restart is required; Windows should resume this installer after you sign back in." >&2; exit 1; }
    run_ps_admin "wsl --install --from-file \"\$env:TEMP\\nixos.wsl\"" || { echo "note: NixOS-WSL installation reported a restart or manual completion is required; Windows should try to resume after sign-in." >&2; exit 1; }
    run_ps "wsl -d NixOS --exec /bin/sh -lc 'printf ready'" >/dev/null 2>&1 || { echo "note: the NixOS-WSL guest is not ready yet; sign back into Windows, start NixOS-WSL, then rerun this script inside the guest if it does not continue automatically." >&2; exit 1; }
  ...
  collect_install_options "winona" "a-pc" "winona" "winnie@winneon.moe" "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIClBpas/q9BP1YNEKQ1w7bHm2RjTJfIimOUWBHekHjoJ" "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIClBpas/q9BP1YNEKQ1w7bHm2RjTJfIimOUWBHekHjoJ" "" apc
